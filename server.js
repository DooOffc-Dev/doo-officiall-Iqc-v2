...
async function render(text, time) {
    await ensureAssets();
...
app.post('/generate', async (req, res) => {
    try {
        ...
        const imageBuffer = await render(text, time);
        res.setHeader('Content-Type', 'image/png');
        res.setHeader('Content-Disposition', 'attachment; filename="iqc-quote.png"');
...
ensureAssets().then(() => {
    app.listen(PORT, () => {
        console.log(`🔥 IQC API running on port ${PORT}`);
